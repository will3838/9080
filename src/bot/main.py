...
from bot.handlers.captcha import captcha_answer_message
from bot.handlers.help import help_command
from bot.handlers.inventory import inventory_callback, inventory_command, inventory_text_trigger
from bot.handlers.spin import spin_command
from bot.logging_config import setup_logging
from bot.roulette.roulette_animation import generate_roulette_animation_bytes
from bot.storage.inventory_db import init_inventory_db


logger = logging.getLogger(__name__)


def ensure_item_folder_exists() -> None:
...
def main() -> None:
    ...
    load_dotenv()
    setup_logging()
    ensure_item_folder_exists()
    ensure_item_excel_exists()
    ensure_spin_excel_exists()
    init_inventory_db()
    items = load_items_once()

    logger.info("Generating roulette animation...")
    roulette_animation_bytes = generate_roulette_animation_bytes(items)
    logger.info("Roulette animation generated, size=%s bytes", len(roulette_animation_bytes))

    settings = load_settings()

    application = Application.builder().token(settings.telegram_bot_token).build()
    application.bot_data["items"] = items
    application.bot_data["roulette_animation"] = {
        "bytes": roulette_animation_bytes,
        "duration_s": 7,
        "filename": "roulette.mp4",
        "width": 1280,
        "height": 720,
    }
    application.bot_data.setdefault("spinning_users", set())
    application.bot_data.setdefault("spin_counts", {})
    application.bot_data.setdefault("captcha_pending", {})
    application.add_handler(CommandHandler("help", help_command))
    application.add_handler(CommandHandler("spin", spin_command))
    application.add_handler(CommandHandler("inv", inventory_command))
    application.add_handler(CommandHandler("inventory", inventory_command))
    application.add_handler(CallbackQueryHandler(inventory_callback, pattern=r"^inv:"))
    application.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, captcha_answer_message, block=False))
    application.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, inventory_text_trigger))
