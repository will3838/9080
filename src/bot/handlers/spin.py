...
async def spin_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if user_id in pending:
        ...
        await update.message.reply_text(
            f"сначала реши капчу. Капча: {question} Ответь командой: /captcha <число>"
        )
        return

    user_spin_count = spin_counts.get(user_id, 0)
    if user_spin_count > 0 and user_spin_count % 50 == 0:
        question, answer = generate_captcha()
        pending[user_id] = {"question": question, "answer": answer}
        await update.message.reply_text(
            f"Капча: {question} Ответь командой: /captcha <число>"
        )
        return
    ...
    spin_counts[user_id] = user_spin_count + 1
