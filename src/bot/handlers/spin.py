...
async def spin_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if user_id in captcha_pending:
        ...
        await update.message.reply_text(f"сначала реши капчу. Капча: {question} Ответь командой: /captcha <число>")
        return
    ...
    if current_count > 0 and current_count % 50 == 0:
        ...
        await update.message.reply_text(f"Капча: {question} Ответь командой: /captcha <число>")
        return
