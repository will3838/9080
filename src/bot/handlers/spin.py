...
async def spin_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        animation_data = context.application.bot_data.get("roulette_animation")
        if not animation_data:
            raise RuntimeError("roulette animation is not initialized")

        animation_buffer = io.BytesIO(animation_data["bytes"])
        animation_buffer.name = animation_data["filename"]
        anim_msg = await update.message.reply_animation(
            animation=animation_buffer,
            duration=animation_data["duration_s"],
