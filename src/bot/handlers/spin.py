...
async def spin_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        roulette_animation = context.application.bot_data.get("roulette_animation")
        if roulette_animation is None:
            raise RuntimeError("Roulette animation is not initialized")

        animation_buffer = io.BytesIO(roulette_animation["bytes"])
        animation_buffer.name = roulette_animation["filename"]
        animation_message = await update.message.reply_animation(
            animation=animation_buffer,
            duration=roulette_animation["duration_s"],
            width=roulette_animation["width"],
            height=roulette_animation["height"],
        )
